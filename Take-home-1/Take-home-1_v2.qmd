...
# Step 3: Recreate the neighbor list and spatial weights
listw <- nb2listw(merged_data5_8$nb, style = "W")
nrow(merged_data5_8)
length(listw$neighbours)
...
# Step 3: Recreate the neighbor list and spatial weights
listw <- nb2listw(merged_data112$nb, style = "W")
nrow(merged_data112)
length(listw$neighbours)
...
# Step 3: Recreate the neighbor list and spatial weights
listw <- nb2listw(merged_data47$nb, style = "W")
nrow(merged_data47)
length(listw$neighbours)
...
lag_merged_data6_9$total_trips <- lag.listw(listw2, merged_data6_9$total_trips)
DV69 <- hunan$lag_GDPPC - mean(hunan$lag_GDPPC)     
```
...
plot_69_all <- 
  tm_shape(merged_data6_9.localMI69) +
  tm_fill(
    col = "Ii",
    style = "cat",
    palette = "Spectral"
...
mean_69 <- merged_data6_9.localMI69  %>%
  filter(Ii <= 0.05)

mean_69_p <- merged_data6_9.localMI69  %>%
  filter(Pr.Ii <= 0.05)
...
plot_69_mean <- 
  ...
  tm_layout(
    main.title = "Map 2: Local Moran Clusters (P value is 0.05 or lesser)",
    main.title.size = 1,
    main.title.position = "center",
    frame = FALSE)
...
plot_69_mean_p <- 
  ...
  tm_layout(
    main.title = "Map 3: Local Moran Clusters Simulated (P value is 0.05 or lesser)",
    main.title.size = 1,
    main.title.position = "center",
    frame = FALSE)

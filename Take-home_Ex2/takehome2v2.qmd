...
hexagon_busstop_tidy <- hexagon_busstop %>%
  st_drop_geometry() %>%
  select(FID, FINANCE_COUNT, BUSINESS_COUNT, RETAIL_COUNT,TRAIN_COUNT,HDB_COUNT,BUS_STOP_COUNT)
...
FINANCE_COUNT and TRAIN_COUNT seems to be correlated (R = 0.77),we shall remove FINANCE_COUNT in the modelling.
...
In this report, we embarked on a comprehensive analysis of urban mobility patterns, specifically focusing on Singapore's public bus commuter flows during the weekday morning peak hours. Utilizing advanced Spatial Interaction Models (SIMs), we delved into the complex dynamics of public transportation, aiming to understand the factors influencing commuter behavior and transit usage.
